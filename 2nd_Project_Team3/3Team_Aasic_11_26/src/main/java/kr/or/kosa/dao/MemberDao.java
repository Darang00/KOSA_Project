...
//				sql1 = "select idx,userid,title,to_char(writedate),lovenum from board where idx = ? and userid = ?";
				sql1 = "select idx,userid,title,to_char(writedate,'yyyy-MM-dd') as writedate from board where idx = ? and boardstatus=1";
				pstmt1 = conn.prepareStatement(sql1);
				pstmt1.setInt(1,bookmarks.getIdx());
...
//						int lovenum = rs1.getInt("lovenum");
		...
		public List<Bookmarks> BookmarksList(int cpage, int pagesize,String userid){
			/* String sql1 = "select * from bookmarks where userid = ? order by idx desc " //예빈언니가 문제 없다고 했음 내 책임 아님
							+ "where num <= ? " + // end
							"and num >= ?"; // start 
			//+ "and boardstatus =1"; */
			...
			try {
				System.out.println("BookmarksList의 try문 진입");
				conn = ds.getConnection();
				System.out.println("conn 완료");
				pstmt1 = conn.prepareStatement(sql1);
				System.out.println("pstmt1 완료");
				int start = cpage * pagesize - (pagesize - 1);
				int end = cpage * pagesize;
				...
				pstmt1.setInt(2, end);
				pstmt1.setInt(3, start);
				System.out.println("pstmt에 set 완료");
				rs1 = pstmt1.executeQuery();
				System.out.println("rs1 완료");
				
				while (rs1.next()) {
					System.out.println("BookmarksList의 try문 다음 while문 진입");
					//Bookmarks bookmarks = new Bookmarks();
					Bookmarks bookmarks = new Bookmarks();
					System.out.println("BookmarksList의 try문 다음 while 문 Bookmark dto생성");
