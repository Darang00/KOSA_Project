...
public class UserDao {
	...
	// 회원테이블 전체조회
	/*
	 * public List<User> getAllTableList() { try { conn = ds.getConnection();
	 * 
	 * //dtoList = new ArrayList<User>();
	 * 
	 * sql = "select id, ip from users"; pstmt = conn.prepareStatement(sql); rs =
	 * pstmt.executeQuery();
	 * 
	 * while (rs.next()) {
	 * 
	 * Dto = new User(); Dto.setId(rs.getString("id"));
	 * Dto.setIp(rs.getString("ip")); dtoList.add(Dto);
	 * 
	 * } } catch (Exception e) { System.out.println(e.getMessage()); } finally { try
	 * { pstmt.close(); rs.close(); conn.close(); } catch (Exception e2) {
	 * 
	 * }
	 * 
	 * } return dtoList; }
