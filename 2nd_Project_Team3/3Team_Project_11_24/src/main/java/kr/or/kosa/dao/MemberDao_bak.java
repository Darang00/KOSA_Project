...
public class MemberDao_bak {
	...
	public MemberDao_bak() throws NamingException {
	...
	public List<Board> listByCategory(int cpage, int pagesize, String boardname){
		try {
			conn = ds.getConnection();
			sql1 = "select *, to_char(writedate, 'yyyyddmm') from (select rownum as num, b.* from " +
	...
	public int bookmarkUpdate(Bookmarks bookmarks) {
		...
		try {
			...
			if(bookmarkcheck == 0) {
				...
				pstmt1.setString(4, bookmarks.getWritedate());
